...
function kairoDir(): string {
  return process.env.KAIRO_TOKEN_CACHE_DIR ?? join(homedir(), ".kairo");
}

export function sessionFilePath(baseUrl: string): string {
...
async function loadCache(baseUrl: string): Promise<string | null> {
  try {
    const raw = await readFile(sessionFilePath(baseUrl), "utf8");
    const parsed = JSON.parse(raw) as CachedCredentials;
    if (parsed.baseUrl !== baseUrl || !parsed.accessToken) return null;
...
export async function clearCachedToken(baseUrl: string): Promise<void> {
  try {
...
export async function loadValidCachedToken(baseUrl: string): Promise<string | null> {
  const cached = await loadCache(baseUrl);
  if (!cached) return null;
  if (!(await validateToken(baseUrl, cached))) return null;
  return cached;
}
...
async function writePendingDevice(pending: PendingDevice): Promise<void> {
  await mkdir(kairoDir(), { recursive: true });
  await writeFile(pendingDevicePath(), JSON.stringify(pending, null, 2), "utf8");
}

export async function clearPendingDevice(): Promise<void> {
  try {
...
async function requestDeviceCode(baseUrl: string): Promise<DeviceStartResponse> {
...
> {
  ...
  throw new Error(
    data.error_description ?? data.error ?? `Error OAuth (${res.status})`,
  );
}
...
export async function runDeviceLogin(baseUrl: string): Promise<DeviceLoginResult> {
  ...
  return {
    ...
    pending: true,
    message:
      `Abrí ${pending.verificationUriComplete} (o ingresá ${pending.userCode} en ${pending.verificationUri}) ` +
