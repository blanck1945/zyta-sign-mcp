...
function openBraveWindows(url: string): boolean {
...
function openBraveUnix(url: string): boolean {
  const commands =
...
function openSystemDefault(url: string): void {
...
export function openBrowser(url: string): void {
  try {
    const opened =
      process.platform === "win32" ? openBraveWindows(url) : openBraveUnix(url);
    if (!opened) openSystemDefault(url);
  } catch {
    try {
