...
async function writeBridgeSession(accessToken: string): Promise<void> {
...
export function startAgentBridge(): Promise<http.Server> {
  const secret = bridgeSecret();
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, HOST, () => {
      process.stderr.write(
        `[kairo-mcp] agent bridge en http://${HOST}:${port} (localhost + secreto compartido)\n`,
      );
      resolve(server);
    });
  });
