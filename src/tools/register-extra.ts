...
export function registerExtraKairoTools(server: McpServer) {
    ...
    async ({ documentId, alias }) => {
      try {
        const data = await kairoFetch<unknown>(
          ...
          { method: "PATCH", body: { alias: alias ?? null } },
        );
        return ok(data);
      } catch (e) {
        return fail(e);
  ...
  registerAuthTool(server,
    "kairo_delete_document",
    {
      title: "Eliminar documento",
      description:
        "Elimina un documento que subiste (y sus firmas asociadas). Requiere scope 'write'.",
      inputSchema: {
        documentId: z.string().min(1),
      },
    },
  ...
  registerAuthTool(server,
    "kairo_revoke_signature",
    {
      title: "Revocar firma en documento",
      description:
        "Revoca una firma ya hecha (deja de contar en QR/certificado). Si hay firmas posteriores en la cadena, también se revocan. Requiere scope 'write'. Indicá signatureId o signerName (ej. Roberto Malaver, Mendoza).",
      inputSchema: {
        documentId: z.string().min(1),
        signatureId: z
          .string()
    ...
    async ({ documentId, signatureId, signerName }) => {
      try {
        const data = await kairoFetch<unknown>(
          `/api/documents/${encodeURIComponent(documentId)}/signatures/revoke`,
          {
            method: "POST",
            body: {
  ...
  registerAuthTool(server,
    "kairo_download_signed_pdf",
    {
      title: "Descargar PDF firmado",
      description:
        "Descarga el PDF estampado de una firma. Podés acceder si ves el documento (incluye firmas del estudio).",
      inputSchema: {
    ...
    async ({ signatureId }) => {
      try {
        const { data, contentType, filename } = await kairoFetchBinary(
          `/api/signatures/${encodeURIComponent(signatureId)}/download`,
        );
        return ok({
          signatureId,
          filename,
  ...
  registerAuthTool(server,
    "kairo_get_signature_certificate",
    {
      title: "Obtener certificado de firma",
      description:
        "Devuelve el certificado JSON de una firma (incluye verifyUrl). Requiere ver el documento.",
      inputSchema: {
        signatureId: z.string().min(1),
      },
    },
    async ({ signatureId }) => {
      try {
        const { data, filename } = await kairoFetchBinary(
          `/api/signatures/${encodeURIComponent(signatureId)}/certificate`,
        );
  ...
  registerAuthTool(server,
    ...
    async () => {
      try {
        const data = await kairoFetch<unknown>("/api/signature-presets");
        return ok(data);
      } catch (e) {
        return fail(e);
      }
  ...
  registerAuthTool(server,
    ...
    async ({ presetId, signatureImageBase64, firstName, lastName, dni, cuil }) => {
      try {
        const data = await kairoFetch<unknown>(
          `/api/signature-presets/${encodeURIComponent(presetId)}`,
          {
            method: "PATCH",
            body: { signatureImageBase64, firstName, lastName, dni, cuil },
          },
  ...
  registerAuthTool(server,
    "kairo_delete_studio",
    {
      ...
      description:
        "Elimina un estudio definitivamente. Requiere rol de gestión y scope 'write'.",
      inputSchema: {
        studioId: z.string().min(1),
      },
    },
  ...
  registerAuthTool(server,
    ...
    async ({ studioId, name }) => {
      try {
        const data = await kairoFetch<unknown>(
          `/api/studios/${encodeURIComponent(studioId)}/areas`,
  ...
  registerAuthTool(server,
    ...
    async ({ studioId, areaId, name }) => {
      try {
        const data = await kairoFetch<unknown>(
          `/api/studios/${encodeURIComponent(studioId)}/areas/${encodeURIComponent(areaId)}`,
          { method: "PATCH", body: { name } },
        );
        return ok(data);
      } catch (e) {
  ...
  registerAuthTool(server,
    "kairo_delete_studio_area",
    {
      title: "Eliminar área",
      description:
        "Elimina un área del estudio. Los documentos quedan sin área. Requiere scope 'write'.",
      inputSchema: {
        studioId: z.string().min(1),
        areaId: z.string().min(1),
      },
  ...
  registerAuthTool(server,
    ...
    async ({ studioId, email, role, areaId }) => {
      try {
        const data = await kairoFetch<unknown>(
          `/api/studios/${encodeURIComponent(studioId)}/members`,
          { method: "POST", body: { email, role, areaId } },
        );
        return ok(data);
      } catch (e) {
        return fail(e);
  ...
  registerAuthTool(server,
    ...
    async ({ studioId, memberId, role, areaId }) => {
      try {
        const body: { role?: string; areaId?: string | null } = {};
        if (role !== undefined) body.role = role;
        if (areaId !== undefined) body.areaId = areaId;
        const data = await kairoFetch<unknown>(
          `/api/studios/${encodeURIComponent(studioId)}/members/${encodeURIComponent(memberId)}`,
          { method: "PATCH", body },
        );
        return ok(data);
      } catch (e) {
