...
export function registerSprintKairoTools(server: McpServer) {
  registerAuthTool(server,
    "kairo_get_plan_quota",
    {
      title: "Plan y cuota de firmas",
      description:
        "Devuelve plan (free/premium), features, firmas usadas este mes y límite.",
      inputSchema: {},
    },
    async () => {
      try {
        return ok(await kairoFetch<unknown>("/api/auth/quota"));
      } catch (e) {
        return fail(e);
      }
    },
  ...
  registerAuthTool(server,
    ...
    async ({ pdfBase64, signatureId, certificateJson }) => {
      try {
        ...
        if (signatureId) form.append("signatureId", signatureId);
        if (certificateJson) form.append("certificate", certificateJson);
        return ok(
          await kairoFetch<unknown>("/api/verify", { method: "POST", body: form }),
        );
      } catch (e) {
        return fail(e);
      }
  ...
  registerAuthTool(server,
    "kairo_get_external_bundle_status",
    {
    ...
    async ({ bundleId }) => {
      try {
        return ok(
          await kairoFetch<BundleStatus>(
            `/api/external-sign/bundles/${encodeURIComponent(bundleId)}/status`,
          ),
        );
      } catch (e) {
        return fail(e);
  ...
  registerAuthTool(server,
    "kairo_wait_for_external_bundle",
    {
    ...
    async ({ bundleId, timeoutSeconds, pollIntervalSeconds }) => {
      try {
        ...
        while (Date.now() - started < timeoutMs) {
          polls += 1;
          const status = await kairoFetch<BundleStatus>(
            `/api/external-sign/bundles/${encodeURIComponent(bundleId)}/status`,
          );
        ...
        const last = await kairoFetch<BundleStatus>(
          `/api/external-sign/bundles/${encodeURIComponent(bundleId)}/status`,
        );
  ...
  registerAuthTool(server,
    ...
    async ({ signatureId, pdfBase64 }) => {
      try {
        ...
        return ok(
          await kairoFetch<unknown>(
            `/api/signatures/${encodeURIComponent(signatureId)}/diff`,
    ...
    async ({ signatureId, action }) => {
      try {
        if (action === "download_proof") {
          const { data, filename } = await kairoFetchBinary(
            `/api/signatures/${encodeURIComponent(signatureId)}/ots`,
          );
          return ok({
            signatureId,
            filename,
        ...
        if (action === "upgrade") {
          return ok(
            await kairoFetch<unknown>(
              `/api/signatures/${encodeURIComponent(signatureId)}/anchor`,
              { method: "POST" },
            ),
          );
        }
  ...
  registerAuthTool(server,
    "kairo_list_webhooks",
    { title: "Listar webhooks", inputSchema: {} },
    async () => {
      try {
        return ok(await kairoFetch<unknown>("/api/webhooks"));
      } catch (e) {
        return fail(e);
      }
    },
  ...
  registerAuthTool(server,
    ...
    async ({ url, events, secret }) => {
      try {
        return ok(
          await kairoFetch<unknown>("/api/webhooks", {
            method: "POST",
  ...
  registerAuthTool(server,
    "kairo_get_external_sign_settings",
    { title: "Settings firma externa", inputSchema: {} },
    async () => {
      try {
        return ok(await kairoFetch<unknown>("/api/auth/external-sign-settings"));
      } catch (e) {
        return fail(e);
      }
    },
  ...
  registerAuthTool(server,
    "kairo_update_external_sign_settings",
    {
    ...
    async ({ externalSignLinkTtl, externalSignExpireOnSign }) => {
      try {
        ...
        if (externalSignLinkTtl) body.externalSignLinkTtl = externalSignLinkTtl;
        if (externalSignExpireOnSign !== undefined) {
          body.externalSignExpireOnSign = externalSignExpireOnSign;
        }
        return ok(
          await kairoFetch<unknown>("/api/auth/external-sign-settings", {
            method: "PATCH",
            body,
          }),
        );
    ...
    async (args) => {
      try {
        const uploaded = (await kairoUploadDocument({
          ...
          alias: args.alias,
        })) as { document?: { id: string; filename?: string; alias?: string | null } };

        const documentId = uploaded.document?.id;
        if (!documentId) return fail(new Error("Sin documentId tras upload"));
        ...
        if (!url) return fail(new Error("Sin URL externa"));

        const docName =
          uploaded.document?.alias ?? uploaded.document?.filename ?? args.filename;
        const share = buildShareLinks({
          url,
  ...
  registerAuthTool(server,
    ...
    async () =>
      fail(
        new Error(
          "Plantillas no implementadas. Usá upload + get_sign_url con placement.",
        ),
      ),
  );

  registerAuthTool(server,
    "kairo_create_sequential_sign",
    {
      title: "[Próximamente] Firma secuencial",
      inputSchema: { documentId: z.string().min(1) },
    },
    async () =>
      fail(new Error("Firma secuencial A→B no implementada aún.")),
  );
}
