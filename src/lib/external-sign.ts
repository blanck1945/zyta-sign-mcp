...
export function externalSignRequestBody(args: {
  ...
  if (args.recipientName?.trim()) body.recipientName = args.recipientName.trim();
  if (args.recipientEmail?.trim()) body.recipientEmail = args.recipientEmail.trim();
  if (args.inviteMessage?.trim()) body.inviteMessage = args.inviteMessage.trim();
