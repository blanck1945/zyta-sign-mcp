...
export function registerKairoTools(server) {
    server.registerTool("kairo_login", {
        title: "Iniciar sesión",
        description: "Obligatorio antes de cualquier otra herramienta si no hay sesión. Sin argumentos: abre el navegador en /device " +
        ...
        try {
            ...
            const manual = access_token?.trim() || process.env.KAIRO_API_TOKEN?.trim();
            if (manual) {
                setAccessToken(manual, baseUrl);
                await saveCachedToken(baseUrl, manual);
                const data = await kairoFetch("/api/me");
                const u = data.user;
    ...
    registerAuthTool(server, "kairo_whoami", {
        title: "Quién soy",
        description: "Devuelve el usuario asociado al token del agente (sesión MCP), no la del navegador.",
        inputSchema: {},
    }, async () => {
        try {
            const data = await kairoFetch("/api/me");
    ...
    server.registerTool("kairo_disconnect", {
        title: "Desconectar agente",
        description: "Borra el token cacheado del agente (~/.kairo/). La próxima llamada pedirá autorización de nuevo.",
        inputSchema: {},
    }, async () => {
        try {
    ...
    registerAuthTool(server, "kairo_list_documents", {
        title: "Listar documentos",
        description: "Lista los documentos visibles para el usuario del token, respetando reglas de Estudio (owner/socio/abogado).",
        inputSchema: {
            limit: z
                .number()
                .int()
        ...
        try {
            const data = await kairoFetch("/api/documents");
    ...
    registerAuthTool(server, "kairo_get_document", {
        ...
        try {
            const data = await kairoFetch(`/api/documents/${encodeURIComponent(documentId)}`);
            return ok(data);
        }
        catch (e) {
            return fail(e);
    ...
    registerAuthTool(server, "kairo_download_document", {
        title: "Descargar PDF",
        description: "Descarga el PDF original de un documento y devuelve metadata + bytes en base64.",
        inputSchema: {
            documentId: z.string().min(1),
        },
    }, async ({ documentId }) => {
        try {
            const { data, contentType, filename } = await kairoFetchBinary(`/api/documents/${encodeURIComponent(documentId)}/download`);
    ...
    registerAuthTool(server, "kairo_list_studios", {
        title: "Listar estudios",
        description: "Lista los estudios donde el usuario es miembro junto con su rol y áreas.",
        inputSchema: {},
    }, async () => {
        try {
            const data = await kairoFetch("/api/studios");
            return ok(data);
        }
        catch (e) {
            return fail(e);
    ...
    registerAuthTool(server, "kairo_get_sign_url", {
        title: "Obtener URL para firmar",
        description: "Devuelve una URL para abrir en el navegador y firmar con mouse/touch. " +
        ...
        inputSchema: {
            ...
            mode: z
                ...
                .default("account")
                .describe("account = /sign/{id} con tu cuenta; external = link público."),
    ...
    }, async ({ documentId, mode, recipientName, recipientEmail, inviteMessage, sendEmail, placement, }) => {
        try {
            const doc = await kairoFetch(`/api/documents/${encodeURIComponent(documentId)}`);
            ...
            if (mode === "account") {
                ...
                return ok({
                    ...
                    instructions: "Abrí esta URL en el navegador estando logueado en Zyta. Vas a dibujar la firma (o usar la guardada), ingresar PIN y completar el flujo criptográfico completo.",
            ...
            return ok(enrichExternalSignResponse({
                ...
                instructions: "Abrí la URL en cualquier dispositivo. Compartí por WhatsApp/Telegram con los share links.",
    ...
    registerAuthTool(server, "kairo_wait_for_signature", {
        title: "Esperar firma completada",
        description: "Hace polling hasta que el documento se firme o se agote el tiempo. " +
    ...
    }, async ({ documentId, since, externalRequestId, accountOnly, timeoutSeconds, pollIntervalSeconds, }) => {
        try {
            ...
            while (Date.now() - started < timeoutMs) {
                ...
                if (accountOnly)
                    q.set("accountOnly", "true");
                const status = await kairoFetch(`/api/documents/${encodeURIComponent(documentId)}/sign-status?${q.toString()}`);
                ...
                if (status.externalRequest?.status === "expired") {
                    return ok({
                        ...
                        polls,
                        waitedMs: Date.now() - started,
                        message: "El link de firma externa expiró sin completarse. Generá uno nuevo con kairo_get_sign_url.",
                    });
                }
            ...
            if (since)
                qs.set("since", since);
            if (externalRequestId)
                qs.set("externalRequestId", externalRequestId);
            if (accountOnly)
                qs.set("accountOnly", "true");
            const last = await kairoFetch(`/api/documents/${encodeURIComponent(documentId)}/sign-status?${qs.toString()}`);
            return ok({
                ...last,
                signed: false,
                timedOut: true,
                ...
                polls,
                waitedMs: Date.now() - started,
                message: "Tiempo de espera agotado. Si aún no firmaste, abrí la URL y completá el flujo en el navegador.",
            });
        }
        catch (e) {
            return fail(e);
    ...
    registerAuthTool(server, "kairo_request_external_sign", {
        title: "Pedir firma externa (legacy)",
        description: "Preferí kairo_get_sign_url(mode=external) o kairo_sign_workflow. Crea link externo con share links.",
        inputSchema: {
            documentId: z.string().min(1),
            ...externalSignCommonSchema,
        },
    }, async ({ documentId, recipientName, recipientEmail, inviteMessage, sendEmail, placement, }) => {
        try {
            const doc = await kairoFetch(`/api/documents/${encodeURIComponent(documentId)}`);
            const label = doc.document?.alias ?? doc.document?.filename ?? "documento";
            const data = await kairoFetch(`/api/documents/${encodeURIComponent(documentId)}/external-requests`, {
                method: "POST",
                body: externalSignRequestBody({
                    recipientName,
                    recipientEmail,
    ...
    registerAuthTool(server, "kairo_list_external_requests", {
        title: "Listar firmas externas pendientes",
        description: "Lista los pedidos de firma externa (token efímero) abiertos para un documento.",
        inputSchema: {
            documentId: z.string().min(1),
        },
    }, async ({ documentId }) => {
    ...
    registerAuthTool(server, "kairo_revoke_external_request", {
        title: "Revocar pedido de firma externa",
        description: "Invalida un pedido de firma externa (el link deja de funcionar inmediatamente). Requiere scope 'write'.",
        inputSchema: {
            documentId: z.string().min(1),
        ...
        try {
            const data = await kairoFetch(`/api/documents/${encodeURIComponent(documentId)}/external-requests/${encodeURIComponent(requestId)}`, { method: "DELETE" });
            return ok(data);
        }
        catch (e) {
            return fail(e);
    ...
    registerAuthTool(server, "kairo_create_studio", {
        title: "Crear estudio",
        description: "Crea un nuevo estudio (requiere plan Premium y scope 'write'). Devuelve el estudio creado.",
        inputSchema: {
    ...
    registerAuthTool(server, "kairo_verify_signature", {
        title: "Verificar firma por hash",
        description: "Verifica una firma de Zyta a partir del hash del documento. No requiere subir el PDF.",
        inputSchema: {
            documentHash: z.string().regex(/^[a-f0-9]{64}$/i),
        },
    }, async ({ documentHash }) => {
        try {
            const data = await kairoFetch(`/api/verify?hash=${encodeURIComponent(documentHash)}`);
            return ok(data);
        }
        catch (e) {
            return fail(e);
...
async function main() {
    ...
    console.error(`[kairo-mcp] listo (stdio). Base: ${getBaseUrl()} — usá kairo_login si no hay sesión.`);
}
const isDirectRun = (() => {
